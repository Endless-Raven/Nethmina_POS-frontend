...
function AdminProductReturns() {
  ...
  const [instockModalOpen, setIistockModalOpen] = useState(false);
  ...
  const [instockmodalData, setInstockModalData] = useState({
    return_id: null,
    expense_category: "",
    expense_amount: "",
    user_id: "",
  ...
  const handleInstockReturn = (return_id) => {
    fetch(`${API_BASE_URL}/return/InstockTostock`, {
      method: "POST",
      headers: {
  ...
  const handleInstockConfirmReturn = (return_id) => {
    fetch(`${API_BASE_URL}/return/instock/InstockReturn`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleInstockSubmitExpense =  () => {
    fetch(`${API_BASE_URL}/return/processInStockReturn/WithExpense`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(instockmodalData),
    })
      .then((response) => response.json())
      .then((data) => {
        alert("Expense and return processed successfully!");
        fetchData("return/returns");
        setIistockModalOpen(false);
      })
      .catch((error) => {
  ...
  const handleInStockOpenModal = (return_id) => {
    setInstockModalData((prev) => ({
      ...prev,
      return_id,
      user: userData.user_id,
      store_id: userData.store_id, // Assuming store_id comes from userData
    }));
    setIistockModalOpen(true);
  };
                    ...
                    <td className="border px-4 py-2 flex gap-2">
                      {title === "Pending Returns" ? (
                        <>
                          <button
                            onClick={() =>
                              handleInstockReturn(returnRecord.return_id)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleInstockConfirmReturn(returnRecord.return_id)
                            }
                          ...
                          <button
                            onClick={() =>
                              //handleInstockSubmitExpense(returnRecord.return_id)
                              handleInStockOpenModal(returnRecord.return_id)
                            }
                        ...
                        <>
                          <button
                            onClick={() =>
                              handleReturnToStock(returnRecord.return_id)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleConfirmReturn(returnRecord.return_id)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleOpenModal(returnRecord.return_id)
                            }
                            className="bg-yellow-500 text-white px-4 py-2 rounded"
                          >
                            Add Expense
  ...
  return (
    <div className="p-4">
      <div className="space-y-8">
        {renderTable(
          "Pending Returns",
          (r) => r.status === "pending" && r.stock_status === "In Stock",
          "red",
          "text-red-600",
          true
        )}
        {/* Pending Returns: Not In Stock */}
        {renderTable(
          "Pending Returns (Not In Stock)",
          (r) => r.status === "pending" && r.stock_status === "Not In Stock",
          "red",
          "text-red-600",
          true
        )}
      ...
      {instockModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-md shadow-lg w-[90%] max-w-md">
            <h2 className="text-lg font-bold mb-4">Add In Stock Return Expense Details</h2>
            <div className="space-y-4">
              <div>
                <label className="block mb-2 font-medium">
                  Expense Category
                ...
                <input
                  type="text"
                  className="w-full px-3 py-2 border rounded-md"
                  value={instockmodalData.expense_category}
                  onChange={(e) =>
                    setInstockModalData((prev) => ({
                      ...prev,
                      expense_category: e.target.value,
              ...
              <div>
                ...
                <input
                  type="number"
                  className="w-full px-3 py-2 border rounded-md"
                  value={instockmodalData.expense_amount}
                  onChange={(e) =>
                    setInstockModalData((prev) => ({
                      ...prev,
                      expense_amount: e.target.value,
            ...
            <div className="flex justify-end mt-4 gap-2">
              <button
                className="px-4 py-2 bg-gray-400 text-white rounded"
                onClick={() => setIistockModalOpen(false)}
              >
                Cancel
              </button>
              <button
                className="px-4 py-2 bg-green-500 text-white rounded"
                onClick={handleInstockSubmitExpense}
              >
                Submit
              </button>
            </div>
